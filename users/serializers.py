...
class SignUpSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        # user -> email -> emailiga jonatish kerak agar user -> phone -> kodi teliga jonatishi kerak. Uning ushun:
    ...
    def validate_email_phone_number(self, value):
        ...
        if value and User.objects.filter(email=value).exists():
            data = {
                "success": False,
                "message": "Bu email avval ishlatilgan"
            }
            raise ValidationError(data)
        elif value and User.objects.filter(phone_number=value).exists():
            data = {
                "success": False,
                "message": "Bu telefon raqam avval ishlatilgan"
            }
            raise ValidationError(data)
...
class ChangeUserInformation(serializers.Serializer):
    ...
    def validate_first_name(self, first_name):
        if len(first_name) < 2 or len(first_name) > 30:
            raise ValidationError(
                {
                    "message": "first_name must be between 2 and 30 characters long"
                }
            )
        if first_name.isdigit():
            raise ValidationError(
                {
                    "message": "The first_name is entirely numeric"
                }
            )
        return first_name
    ...
    def validate_last_name(self, last_name):
        if len(last_name) < 2 or len(last_name) > 30:
            raise ValidationError(
                {
                    "message": "last_name must be between 2 and 30 characters long"
                }
            )
        if last_name.isdigit():
            raise ValidationError(
                {
                    "message": "The last_name is entirely numeric"
                }
            )
        return last_name
...
class LoginSerializer(TokenObtainPairSerializer):
    ...
    def auth_validate(self, data):
        ...
        if check_user_type(user_input) == 'username':
            ...
            data = {
                'success': True,
                'message': "Siz email, username yoki telefon raqam jonatishingiz kerak"
            }
            raise ValidationError(data)
        ...
        if user is not None:
            ...
            raise ValidationError(
                {
                    'success': False,
                    'message': "Sory, login or password you entered is incorrect. Please check and try again!"
                }
            )
    def validate(self, data):
